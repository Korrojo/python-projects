...
class MongoDBConnector:
    ...
    def __init__(
        ...
        if mongodb_uri:
            ...
            if not uri_value:
                raise ValueError(f"MongoDB URI not found. Set {uri_var} or MONGODB_URI in shared_config/.env")
        ...
        if database_name:
            ...
            if not db_value:
                raise ValueError(f"Database name not found. Set {db_var} or DATABASE_NAME in shared_config/.env")
...
def get_mongo_client(
    env: str = "DEV", mongodb_uri: Optional[str] = None, **kwargs
