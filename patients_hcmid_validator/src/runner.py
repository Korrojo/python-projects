...
class ExcelDictReader:
    ...
    def __enter__(self):
        ...
        self.fieldnames = [str(cell).strip() if cell is not None else f"Column_{i}" for i, cell in enumerate(first_row)]
        return self
    ...
    def __next__(self):
        if self._rows_iter is None or self.fieldnames is None:
            raise StopIteration
...
def run_validation(
    ...
    with connection:
        ...
        with file_reader as f_in:
            if input_format == "excel":
                # For Excel, f_in is already the ExcelDictReader (acts as Dict iterator)
            ...
            for i, row in enumerate(reader, start=1):
                # Ensure row is a dict (type guard for mixed reader types)
