...
For each entry in the CSV, use AthenaAppointmentId to locate the corresponding appointment
(Slots.Appointments.AthenaAppointmentId) in the StaffAvailability collection. Validate that the following fields match
between Athena(csv) and Ubiquity(mongo):
