...
def parse_args():
    p = argparse.ArgumentParser(description="Copy indexes from a source collection to a target collection in the same DB.")
...
def main():
    ...
    try:
        ...
        for idx in indexes:
            ...
            if args.dry_run:
                print(f"DRY-RUN: would create index '{opts.get('name')}' keys={keys} options={ {k:v for k,v in opts.items() if k!='name'} }")
                if collation_error:
                    print(f"  NOTE: source index had collation but it could not be converted; review manually")
                continue
