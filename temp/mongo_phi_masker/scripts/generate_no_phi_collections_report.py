...
OUTPUT_CSV_FILE = os.path.join(project_dir, f"docs/nophi_collections/no_phi_collections_report_{datetime.now().strftime('%Y%m%d')}.csv")
...
def parse_args():
    ...
    parser.add_argument('--collections', default=NO_PHI_COLLECTIONS_FILE,
                      help='Path to no-phi collections JSON file')
    parser.add_argument('--output', default=OUTPUT_CSV_FILE,
                      help='Path to output CSV file')
    parser.add_argument('--env', default=ENV_FILE,
                      help='Path to environment file')
    parser.add_argument('--database', 
                      help='MongoDB database name to use (overrides MONGO_SOURCE_DB)')
...
def main():
    ...
    if not database_name:
        logger.error("Database name not specified. Please provide it with --database or set MONGO_SOURCE_DB environment variable.")
        return 1
