...
1. **Mask/anonymize** PHI fields to comply with HIPAA and healthcare data privacy regulations
1. **Migrate** masked data between MongoDB environments (Production → Staging/Testing)
...
1. **Source-to-Destination**: Copy documents from source to destination while masking (safer, preserves original)
1. **Incremental Processing**: Support for checkpoint-based resumable processing
...
1. Patient Identity (many PHI fields - e.g., Patients, PatientPanel)
1. Appointments & Scheduling
1. Care Plans & Medical Records
...
- **Action**:
  ...
  1. Keep `masking.py` temporarily for backward compatibility
...
- Files: test_connection.py, test_dob_masking.py, test_env.py, test_fax_masking.py, test_masking_sample.py,
  test_masking_simple.py, test_mongo_connection.py, test_rule_groups_offline.py
...
1. ✅ **Update orchestrate_migration.py**:
...
**Recommendation**: Proceed with cleanup in phases as outlined above. The core functionality is solid, but removing
unused dependencies will improve:
