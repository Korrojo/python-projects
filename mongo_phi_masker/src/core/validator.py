...
class MaskingValidator:
    ...
    def validate_document(
        self,
        masked_doc: Dict[str, Any],
        original_doc: Dict[str, Any] = None
    ...
    def _validate_structure(
        ...
        for key in original_doc:
            ...
            if key not in masked_doc:
                issues.append({
                    "field": current_path,
                    "issue": "Field missing in masked document"
                })
                continue
                
            # Recursively validate nested documents
            if isinstance(original_doc[key], dict) and isinstance(masked_doc[key], dict):
                nested_issues = self._validate_structure(
                    masked_doc[key], original_doc[key], current_path
                )
        ...
        for key in masked_doc:
            ...
            if key not in original_doc and not key.startswith("_"):
                issues.append({
                    "field": current_path,
                    "issue": "Extra field in masked document"
                })
            ...
            if current_path in self.fields_to_validate or key in self.fields_to_validate:
                ...
                if isinstance(value, str):
                    if self._is_potential_phi(value):
                        issues.append({
                            "field": current_path,
                            "issue": "Field may contain unmasked PHI",
                            "value": value
                        })
...
class ValidationProcessor:
    ...
    def __init__(
        self,
        fields_to_validate: List[str] = None,
        pattern_definitions_path: str = None,
        logger: logging.Logger = None
    ):
        ...
        self.validator = MaskingValidator(
            fields_to_validate=self.fields_to_validate,
            patterns_to_detect=self.patterns,
            logger=self.logger
        )
    ...
    def validate_batch(
        ...
        original_docs: List[Dict[str, Any]] = None
