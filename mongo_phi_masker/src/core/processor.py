...
from ..utils.resource_monitor import ResourceMetricsWriter, ResourceMonitor
...
class DataProcessor:
    ...
    def __init__(self, config: dict[str, Any], logger: logging.Logger = None):
        ...
        # Set up validation
        self.validator = ValidationProcessor(fields_to_validate=self._get_phi_field_names(), logger=self.logger)
        ...
        self.resource_monitor = ResourceMonitor(config=resources_config, metrics_callback=self._handle_metrics_update)
        ...
        self.logger.info(
            f"Initialized processor with batch size {self.initial_batch_size} " f"and {self.worker_count} workers"
        )
    ...
    def process_collection(self, query: dict[str, Any] = None, incremental: bool = False) -> dict[str, Any]:
    ...
    def _process_documents(self, query: dict[str, Any] = None) -> dict[str, Any]:
        ...
        try:
            ...
            cursor = self.source_connector.find_documents(query=query, batch_size=self.current_batch_size)
    ...
    def _process_batch(self, batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        # Validate documents
        validation_results, validation_issues = self.validator.validate_batch(masked_docs, batch)
        ...
        if self.masking_mode == "separate":
            ...
            for _i, doc in enumerate(masked_docs):
                if "_id" in doc:
                    self.source_connector.update_document(query={"_id": doc["_id"]}, update={"$set": doc})
    ...
    def process_in_situ(
        self, query: dict[str, Any] = None, batch_size: int = None, dry_run: bool = False
    ) -> dict[str, Any]:
        ...
        try:
            ...
            self.masking_processor.get_ruleset()
