        ...
        while not self.stop_event.is_set():
            try:
                ...
                if prev_state != self.resource_state:
                    self.logger.info(f"Resource state changed from {prev_state} to {self.resource_state}")
...
class ResourceMetricsWriter:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.system_filename_pattern = system_config.get("filename", "system_stats_%Y%m%d_%H%M%S.csv")
        ...
        self.batch_filename_pattern = batch_config.get("filename", "batch_stats_%Y%m%d_%H%M%S.csv")
