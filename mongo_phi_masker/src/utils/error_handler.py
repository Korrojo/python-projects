...
class ErrorHandler:
    ...
    def _handle_application_error(self, exc: MaskerError, context: dict[str, Any]) -> None:
        ...
        """
        error_info = {"message": exc.message, "code": exc.code, "details": exc.details, "context": context}
    ...
    def _handle_system_error(self, exc: Exception, context: dict[str, Any]) -> None:
        ...
        error_info = {"type": error_type, "message": error_message, "stack": error_stack, "context": context}
        ...
        def wrapper(*args, **kwargs):
            ...
            while attempt <= max_attempts:
                try:
                    ...
                    logger.warning(
                        f"Retry {attempt}/{max_attempts} for {func_name} " f"due to {type(e).__name__}: {str(e)}"
                    )
...
def setup_global_exception_handler(error_handler: ErrorHandler) -> None:
    ...
    def global_exception_hook(exc_type, exc_value, exc_traceback):
        ...
        error_handler.handle_exception(
            exc_value, {"type": exc_type.__name__, "traceback": traceback.format_tb(exc_traceback)}
        )
