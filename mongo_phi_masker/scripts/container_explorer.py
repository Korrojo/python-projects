    ...
    if isinstance(doc, dict):
        ...
        if all(isinstance(item, dict) for item in doc):
            ...
            result["__array_of_documents__"] = {
                "__length__": len(doc),
                "__samples__": [
                    explore_structure(doc[i], f"{path}[{i}]", max_depth, current_depth + 1) for i in range(samples)
                ],
            }
        else:
        ...
        return {"__type__": type(doc).__name__, "__value__": str(doc)[:50]}
...
def count_phi_fields(doc, phi_fields):
    ...
    counts = dict.fromkeys(phi_fields, 0)
    doc_presence = dict.fromkeys(phi_fields, False)
    ...
    for field in phi_fields:
        if "[]" in field:
            ...
            if isinstance(base_value, list):
                for _i, item in enumerate(base_value):
...
def analyze_container_structure(sample_size=10, logger=None):
    ...
    phi_counts = dict.fromkeys(phi_fields, 0)
    doc_counts = dict.fromkeys(phi_fields, 0)
    ...
    for field in phi_fields:
        logger.info(f"{field:<50} {doc_counts[field]:<10} {phi_counts[field]:<12}")
