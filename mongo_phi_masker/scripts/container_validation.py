...
def validate_field_value(source_value, dest_value, field_config):
    ...
    if validation_type == "replace_string":
        ...
        return (dest_value is not None and 
                isinstance(dest_value, str) and
                dest_value.isupper() and 
...
def validate_container_masking(sample_size=100, logger=None):
    ...
    field_validations = [
        ...
        {"path": "Encounter[].PhoneCall.Activities[].PhoneNumber", "type": "random_10_digit_number", "pattern": r"^\d{10}$"},
        {"path": "Encounter[].AssessmentAndPlan.ProblemList[].Comments", "type": "replace_string", "value": "xxxxxxxxxx"},
        {"path": "Encounter[].AssessmentAndPlan.ProblemList[].FinalNotes", "type": "replace_string", "value": "xxxxxxxxxx"},
        {"path": "Encounter[].Dat[].OtherReason", "type": "replace_string", "value": "xxxxxxxxxx"},
        {"path": "Encounter[].Document.Documents[].Path", "type": "replace_path", "value": "//vm_fs01/Projects/EMRQAReports/sampledoc.pdf"}
    ]
    
    # Initialize field validation counters
    for field_config in field_validations:
        path = field_config["path"]
        validation_results["field_validation"][path] = {
            "checked": 0,
            "passed": 0,
            "failed": 0,
            "not_found": 0
    ...
    for doc_id in sample_ids:
        ...
        for field_config in field_validations:
            ...
            if len(source_values) != len(dest_values):
                logger.warning(f"Document {doc_id}: Field {path} has {len(source_values)} values in source but {len(dest_values)} in destination")
                doc_passed = False
                validation_results["field_validation"][path]["failed"] += len(source_values)
                continue
            
            # Validate each value
            for i, (source_value, dest_value) in enumerate(zip(source_values, dest_values)):
    ...
    for field, stats in validation_results["field_validation"].items():
        ...
        logger.info(f"  {field}: {pass_rate:.1f}% passed ({stats['passed']}/{stats['checked']} checked, {stats['not_found']} not found)")
...
def main():
    ...
    parser.add_argument('--config', default='config/validation/container_fields.json', help='Path to field validation config')
