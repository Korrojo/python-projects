...
This tool facilitates the migration of MongoDB collections that do not contain Protected Health Information (PHI) from a
source database to a destination database. Since these collections don't contain PHI, they can be directly copied
without any masking or transformation.
...
1. Connect to source and destination MongoDB instances
1. Copy the specified collections in batches
1. Track migration progress and generate logs
...
Create a text file at `docs/no-phi_collections.txt` with one collection name per line. Lines starting with `#` are
...
1. **Batch Inserts**: Documents are inserted in batches using `insert_many()` for better performance.
1. **Worker Thread Control**: The number of parallel workers can be controlled via:
...
After migration, a log file is generated at `docs/migration_log.json` with details about each collection's migration
status, document count, and any errors encountered.
...
1. **Permission Issues**: Ensure the MongoDB user has read access to source and write access to destination
1. **Collection Already Exists**: By default, collections that already exist in the destination are skipped
...
This script is part of the MongoPHIMasker project, which handles PHI data masking for MongoDB databases to ensure HIPAA
compliance. While the main project focuses on masking PHI data, this script provides a faster path for non-PHI
collections that don't require masking.

For collections containing PHI data, use the main masking process with appropriate rule groups based on the collection
category.
